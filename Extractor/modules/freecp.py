...
def normalize_cpwp_url(url):
    """
    Fixes malformed Classplus media URLs and decodes base64 paths if detected.
    Accepts both direct and encoded links.
    Preserves query string for signed m3u8 links.
    """
    url = url.split('*')[0]
    # Do NOT strip query string if ?key= is present
    if "?key=" not in url:
        url = url.split('?')[0]
    # Fix missing slash after .com
    if url.startswith('https://media-cdn.classplusapp.com') and not url.startswith('https://media-cdn.classplusapp.com/'):
        url = url.replace('https://media-cdn.classplusapp.com', 'https://media-cdn.classplusapp.com/', 1)
    # If base64 after .com/
    path = url[len('https://media-cdn.classplusapp.com/'):]
    if path and path[0] == 'L':
        try:
            b64 = path
            if b64.startswith('L'):
                b64 = b64[1:]
            ...
            if decoded.startswith('/'):
                decoded = decoded[1:]
            url = 'https://media-cdn.classplusapp.com/' + decoded
        except Exception:
            pass
...
async def process_cpwp_url(url_val: str, name: str, session: aiohttp.ClientSession, headers: Dict[str, str]) -> str | None:
    try:
        url_val = normalize_cpwp_url(url_val)
        # If already signed m3u8, return as is
        if url_val.endswith('.m3u8') and ('key=' in url_val and 'hdnts=' in url_val):
            return f"{name}:{url_val}\n"
        # Old logic for legacy/unsigned links:
...
async def get_cpwp_course_content(session: aiohttp.ClientSession, headers: Dict[str, str], Batch_Token: str, folder_id: int = 0, limit: int = 9999999999, retry_count: int = 0) -> Tuple[List[str], int, int, int]:
    ...
    for (_, task), result in zip(content_tasks, content_results):
